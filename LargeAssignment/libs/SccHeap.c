      ...
      if (cur->size >= (alignedSize + RESIDUE + sizeof(heap_size_t))) { 
        /*
          When the memory block is large enough after cutting, 
            just cut (alignedSize + sizeof(heap_size_t)) bytes of memory from the end of the block.当切割后的内存块足够大时、 
            只需从内存块的末尾剪切 (alignedSize + sizeof(heap_size_t)) 字节即可
             (1) alignedSize bytes of memory will be returned and used as heap memory.(1) 将返回并用作堆内存的 alignedSize 字节。
          ...
          Q1. Reset cur->size to be (cur->size - alignedSize - sizeof(heap_size_t)).把size更新成分配之后的大小
